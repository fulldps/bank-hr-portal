...
async function handleStatusChange(e: Event) {
  ...
  await store.updateTicket(ticket.value!.id, { status: newStatus });
}
      ...
      <BaseCard>
        <div class="p-6 space-y-6">
          ...
          <div v-if="ticket.description">
            ...
            <p
              class="text-sm text-bnb-text leading-relaxed bg-bnb-bg rounded-lg px-4 py-3"
            >
              {{ ticket.description }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 pt-4 border-t border-bnb-border">
            ...
            <div>
              ...
              <div v-if="ticket.author" class="flex items-center gap-2 h-9">
                <div
                  class="w-6 h-6 rounded-full bg-bnb-primary/60 flex items-center justify-center flex-shrink-0"
                >
                  <span class="text-white text-[9px] font-bold">
                    {{
                      ticket.author.name
