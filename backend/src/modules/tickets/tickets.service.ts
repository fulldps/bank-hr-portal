...
export async function getTickets(filters: {
  ...
  return rows.map((r) => ({
    ...
    assignee: r.assignee_id
      ? { id: r.assignee_id, name: r.assignee_name }
      : null,
  }));
}
...
export async function getTicketById(id: string) {
  const row = await queryOne(
    `SELECT t.*, u1.full_name as author_name, u2.full_name as assignee_name
     FROM tickets t
     LEFT JOIN users u1 ON t.author_id = u1.id
     LEFT JOIN users u2 ON t.assignee_id = u2.id
     WHERE t.id = $1`,
  ...
  if (!row) throw new AppError(404, "Тикет не найден", "NOT_FOUND");
...
export async function createTicket(data: {
  ...
  const row = await queryOne(
    `INSERT INTO tickets
       (title, description, priority, status, author_id, assignee_id, due_date, tags)
...
export async function deleteTicket(id: string) {
  const row = await queryOne("DELETE FROM tickets WHERE id = $1 RETURNING id", [
    id,
  ]);
  if (!row) throw new AppError(404, "Тикет не найден", "NOT_FOUND");
  return true;
